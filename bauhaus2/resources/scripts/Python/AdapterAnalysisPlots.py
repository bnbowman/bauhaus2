...
def ReadDataByAdapter( fn ):
    ...
    with open( fn ) as handle:
        ...
        conIdx   = columns.index("Condition")
        for line in handle:
            ...
            rec = (hn, zAcc, cAcc, isReal, isHit, con)
...
def CallAccuracyPlots( name, data ):
    ...
    for adp, vals in data.iteritems():
        ...
        condition = [v[5] for v in filtered]
        raw = {"AdapterType": adp,
    ...
    df.to_csv("callAccuracy.csv", encoding='utf-8', index=False)
...
def ZmwAccuracyPlots( name, data ):
    ...
    df.to_csv("zmwAccuracy.csv", encoding='utf-8', index=False)
