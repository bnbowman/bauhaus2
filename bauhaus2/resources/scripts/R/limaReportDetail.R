...
reportFiltered = report %>% filter(Barcoded) %>% filter(BarcodePair %in% unique_bps$BarcodePair) %>% mutate(Filter = "NONE", ScoreLeadSecond = ifelse(ScoreLeadSecond==-1,NA,ScoreLeadSecond), ScoreLeadFirst = ifelse(ScoreLeadFirst==-1,NA,ScoreLeadFirst))
...
g = ggplot(bind_rows(reportFiltered, reportFiltered_pass), aes(group = BarcodePair)) +
  facet_wrap(~BarcodePair, scales = "free_y", ncol = 4)+
  geom_freqpoly(binwidth=5, aes(x = ScoreLeadFirst, group=Filter, color=Filter, lty="First"),alpha=.5)+
  theme_light()+
  scale_linetype_manual(name = "Score Lead",values=c("First"=1,"Second"=2))+
  ylab("Number of ZMWs")+xlab("Score Leads")
if (length(table(reportFiltered$ScoreLeadSecond)) > 0) g = g+geom_freqpoly(binwidth=5, aes(x = ScoreLeadSecond, group=Filter, color=Filter, lty="Second"),alpha=.5)
