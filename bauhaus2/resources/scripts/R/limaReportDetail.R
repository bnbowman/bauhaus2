...
reportFiltered = report %>% filter(Barcoded) %>% filter(BarcodePair %in% unique_bps$BarcodePair) %>% mutate(Filter = "NONE", ScoreLead = ifelse(ScoreLead==-1,NA,ScoreLead), ScoreLead = ifelse(ScoreLead==-1,NA,ScoreLead))
...
readYield = report %>% filter(Barcoded) %>% filter(BarcodePair %in% unique_bps$BarcodePair) %>% select(BarcodePair, ReadLengths, PassedFilters) %>% unnest(ReadLengths) %>% group_by(BarcodePair,PassedFilters) %>% count(BarcodePair, PassedFilters) %>% mutate(Filter = PassedFilters) %>% mutate(Filter=ifelse(Filter==0,"NONE","PASS"))
zmwYield = report %>% filter(Barcoded) %>% filter(BarcodePair %in% unique_bps$BarcodePair) %>% select(BarcodePair, ZMW, PassedFilters) %>% count(BarcodePair, PassedFilters) %>% mutate(Filter = PassedFilters) %>% mutate(Filter=ifelse(Filter==0,"NONE","PASS"))
...
dpi = 150
facetHeight = max(nrow(unique_bps),4)/4*5+1
yieldHeight = max(nrow(unique_bps),4)*0.5+3
facetWidth = 5 + min(nrow(unique_bps),4)*5

df <- data.frame()
tp = ggplot(df) + geom_point() + xlim(0, 10) + ylim(0, 10) +
  annotate(
    geom = "text",
    x = 5,
    y = 5,
...
ggsave(paste0("detail_score_vs_yield.",output),tp,width=5,height=5,dpi=dpi,units="cm")
...
#   facet_wrap(~BarcodePair, scales = "free_y", ncol = 4)+
#   geom_freqpoly(binwidth=5, aes(x = ScoreLead, group=Filter, color=Filter, lty="First"),alpha=.5)+
#   theme_light()+
#   scale_linetype_manual(name = "Score Lead",values=c("First"=1,"Second"=2))+
#   ylab("Number of ZMWs")+xlab("Score Leads")
# if (length(table(reportFiltered$ScoreLead)) > 0) g = g+geom_freqpoly(binwidth=5, aes(x = ScoreLead, group=Filter, color=Filter, lty="Second"),alpha=.5)
