...
makeCoverageTitrationTable1 <- function(maskedVariantsDir)
{
  ...
  computeGenomeSize <- function(alignmentSummaryFile)
  {
    ...
    sequenceRegionLines <-
      lines[grep("##sequence-region", lines)]
...
doTitrationPlots <- function(report, tbl)
{
  ...
  nvals <- lapply(variables, function(x)
    length(unique(tbl[, x])))
  ...
  q <-
    (
      ggplot(tbl, aes(
        x = Coverage, y = ConcordanceQV, color = Condition
      )) +
        geom_line() +
  ...
  if (length(variables) >= 2)
  {
    ...
    function(twoVars)
    {
      ...
      title <-
        str_c("Consensus performance by ",
              str_c(twoVars, collapse = " and "))
      q <-
        (
          ggplot(
            tbl,
            ...
            scale_y_continuous("Concordance (QV)") +
            ggtitle(paste(
              "Consensus Performance By", twoVars[1]
            ))
        )
      report$ggsave(paste0(id, ".png"), q, id, title, title)
    })
      ...
      q <-
        (
          ggplot(
            ...
            scale_y_continuous("Concordance (QV)") +
            ggtitle(
              paste0("Consensus Performance By ", threeVars[1], ", ", threeVars[2])
            )
        )
      report$ggsave(paste0(id, ".png"), q, id, title, title)
    })
...
makeResidualsTable <- function(ccsDf, variables)
{
  ...
  print(head(tbl))
  tbl$MaskedVariantsFile = as.character(tbl$MaskedVariantsFile)
  mm <- function(df) {
  ...
  groupNames = c("Condition",
                 ...
                 "Genome",
                 variables)
  varTypeCounts = subset(tbl, Coverage %in% ROUND.COVERAGE) %>% group_by_(.dots = as.list(groupNames)) %>% do(mm = mm(.))
  varTypeCounts = varTypeCounts[!varTypeCounts$mm %in% "NULL",]
  varTypeCounts = rbindlist(lapply(split(varTypeCounts, seq(
    nrow(varTypeCounts)
  )), function(row) {
    cbind(subset.data.frame(row, select = -c(mm)), row$mm)
  }))
  varTypeCounts
}
...
{
  if (nrow(varTypeCounts) > 0 & "ErrorMode" %in% colnames(varTypeCounts)) {
    maxFreq = max((
      varTypeCounts %>% group_by(Condition, Coverage, ErrorMode) %>% summarise(sumFreq = sum(Freq))
    )$sumFreq)
    for (coverage in unique(varTypeCounts$Coverage)) {
      plt <-
        (
          ggplot(
            data = varTypeCounts[varTypeCounts$Coverage == coverage, ],
            mapping = aes(x = ErrorMode, y = Freq, fill = Base)
      ...
      title.fix <-
        sprintf("Residual errors in %dx consensus sequence (fixed y-axis)",
                coverage)
      ...
      title.free <-
        sprintf("Residual errors in %dx consensus sequence (free y-axis)",
                coverage)
      plt.free_y <-
        (plt + facet_grid(facet.formula, scale = "free_y")
         + ggtitle(title.free))
