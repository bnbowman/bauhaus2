...
First, try a couple cases where there's heterogeneity within a condition.
  ...
  $ bauhaus2 -m -w Mapping -t ${BH_ROOT}test/data/bad-cts/nonuniform-conditions-1.csv validate
  ...
  $ bauhaus2 -m -w Mapping -t ${BH_ROOT}test/data/bad-cts/nonuniform-conditions-2.csv validate
...
Try a case where a genome can't be found:
  ...
  $ bauhaus2 -m -w Mapping -t ${BH_ROOT}test/data/bad-cts/unrecognized-genome.csv validate
  Condition table validation error: Reference not found: WillyWonka
  [1]

Try a case where an input can't be found:

  $ bauhaus2 -m -w Mapping -t ${BH_ROOT}test/data/bad-cts/nonexistent-data.csv validate
...
Missing mandatory p_ variables for CoverageTitration workflow
  ...
  $ bauhaus2 -m -w CoverageTitration -t ${BH_ROOT}test/data/lambdaAndEcoli.csv validate
...
Missing genome mask (CoverageTitration workflow)
  ...
  $ bauhaus2 -m -w CoverageTitration -t ${BH_ROOT}test/data/bad-cts/unrecognized-genome-mask.csv validate
